...
void DisjointSets::dsunion(int i, int j) {
  ...
  if (root_i != root_j) {  // j is the leader 
    leader[root_i] = root_j; // j is leader so update the first number of the set given to become J
    root_i = root_j;  // they  both have same set leader
  }
